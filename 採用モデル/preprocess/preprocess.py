...
args = sys.argv
if args[1] == 'train':
    data = pd.read_csv("/Users/200510/Desktop/TN/matchscout/prehire/改善/zip_採用モデル/training/training.csv")
    ...
    data = pd.read_csv("ユーザー抽出データ_20200124.csv")
    data['annualpay'] = np.nan
    data.loc[data['salary_system'] == 'annual', 'annualpay'] = data.loc[data['salary_system'] == 'annual', 'salary']
    data.loc[data['salary_system'] == 'salary', 'annualpay'] = data.loc[data['salary_system'] == 'salary', 'salary'] * 12
mode = pd.read_csv('statistics_values_mode.csv')
searchparameter = pd.read_csv('/Users/200510/Desktop/TN/matchscout/prehire/改善/zip_採用モデル/training/work_searchparameters.csv')
data = pd.merge(data, searchparameter, on = 'work_id', how = 'left')

##類似度行列 : マッチ度の計算に使用
exp_similarity_converter = pd.read_csv('exp_similarity.csv')
licentiate_similarity_converter = pd.read_csv('licentiate_similarity.csv')
...
if args[1] == 'train':
    ...
    data[params].to_csv("preprocess_target.csv")
